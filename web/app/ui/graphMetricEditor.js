...
//             'measure': 'idle'
//           }
// formula: String, `ip;metric [operator(+,-,*,/) ip;metric]*`
// metrics: ['ip;cpu,cpu0,idle', ...]
...
var GraphSelector = React.createClass({
    ...
    handleAppendingMetric: function () {
        var selected = this.state.selected;
        ...
        metric = this.props.ips[0] + ';' + metric.replace(',,', ',');
        var formula = this.refs.metricInput.getValue().trim() + ' ' + metric;
        this.refs.metricInput.setValue(formula);
    },
        ...
        return (
            <div>
                <div className='configList'>
                    {selectors}
                    <i className="fa fa-plus fa-bg fa-transform addMetricBtn"
                       onClick={this.handleAppendingMetric} title="Add"></i>
                </div>
                <div className='metricEditList'>
                    <List>
                        {metricItems}
                    </List>
                </div>
